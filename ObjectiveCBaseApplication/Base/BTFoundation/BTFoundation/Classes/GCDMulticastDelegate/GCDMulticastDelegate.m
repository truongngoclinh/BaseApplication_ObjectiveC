...
@property (nonatomic, strong) NSMutableDictionary *methodSelectorCache;
@property (nonatomic, strong) NSMutableSet *methodSelectorEmptyCache;
...
- (id)init
{
	if ((self = [super init]))
	{
        ...
        self.methodSelectorCache = [NSMutableDictionary dictionary];
        self.methodSelectorEmptyCache = [NSMutableSet set];
	}
	return self;
}
...
- (void)addDelegate:(id)delegate delegateQueue:(dispatch_queue_t)delegateQueue
{
    ...
    [self.methodSelectorEmptyCache removeAllObjects];
}
...
- (NSMethodSignature *)methodSignatureForSelector:(SEL)aSelector
{
    ...
    NSMethodSignature *result = [self.methodSelectorCache objectForKey:selectorKey];

    if (result) {
        return result;
    }

    if ([self.methodSelectorEmptyCache containsObject:selectorKey]) {
	...
	for (GCDMulticastDelegateNode *node in delegateNodes)
	{
        ...
        if (result != nil)
        {
            [self.methodSelectorCache setObject:result forKey:selectorKey];
            return result;
        }
    }

    [self.methodSelectorEmptyCache addObject:selectorKey];
...
- (void)forwardInvocation:(NSInvocation *)origInvocation
{
	...
	for (GCDMulticastDelegateNode *node in delegateNodes)
	{
		...
		if ([nodeDelegate respondsToSelector:selector])
		{
            ...
            __weak id weakNodeDelegate = nodeDelegate;
			
			dispatch_async(node.delegateQueue, ^{ @autoreleasepool {
				[dupInvocation invokeWithTarget:weakNodeDelegate];
			}});
		}
